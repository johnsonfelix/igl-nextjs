...
export default function CouponsPage() {
  ...
  const [offerForm, setOfferForm] = useState({
    ...
    hotelIds: [] as string[],
    ticketIds: [] as string[],
    sponsorTypeIds: [] as string[],
    boothIds: [] as string[],
    membershipPlanIds: [] as string[], // NEW
  });
      ...
      const endpoints = [
        {
          ...
          setter: (d: any) =>
            setHotels(
              (d || []).map((h: any) => ({
                id: h.id,
                name: h.hotelName || h.name || h.title,
              }))
            ),
        },
        {
        ...
        {
          url: "/api/admin/sponsor-types",
          setter: (d: any) =>
            setSponsorTypes(
              (d || []).map((s: any) => ({ id: s.id, name: s.name }))
            ),
        },
        {
          url: "/api/admin/booths",
          setter: (d: any) =>
            setBooths(
              (d || []).map((b: any) => ({
                id: b.id,
  ...
  const resetCouponForm = () => {
    setCouponForm({ code: "", discountType: "FIXED", discountValue: "" });
  ...
  const handleCouponSubmit = async () => {
    if (!couponForm.code || !couponForm.discountValue) {
      alert("Please fill out all coupon fields.");
      return;
    }
    const payload = {
      ...couponForm,
  ...
  const handleOfferSubmit = async () => {
    if (!offerForm.name || !offerForm.percentage) {
      alert("Please provide offer name & percentage.");
      return;
    }
    ...
    const payload: any = {
      ...
      startsAt: offerForm.startsAt
        ? new Date(offerForm.startsAt).toISOString()
        : null,
      endsAt: offerForm.endsAt
        ? new Date(offerForm.endsAt).toISOString()
        : null,
    ...
    if (offerForm.scope === "CUSTOM") {
      ...
      payload.membershipPlanIds = offerForm.membershipPlanIds;
    }
    try {
  ...
  const handleOfferDelete = async (id: string) => {
    ...
    try {
      await fetchJson(`/api/admin/offers/${id}`, { method: "DELETE" });
      await fetchOffers();
    } catch (err: any) {
  ...
  const getScopeIcon = (scope: OfferScope) => {
    const icons: Record<OfferScope, any> = {
      ALL: Gift,
      HOTELS: Building,
      TICKETS: Ticket,
      SPONSORS: Users,
  ...
  const getScopeColor = (scope: OfferScope) => {
    const colors: Record<OfferScope, string> = {
      ...
      SUBSCRIPTIONS: "bg-rose-100 text-rose-700 border-rose-200",
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-indigo-600 to-violet-600 text-white rounded-xl font-medium hover:from-indigo-700 hover:to-violet-700 transition-all shadow-lg shadow-indigo-200"
            >
        ...
        {masterListError && (
          <div className="p-4 bg-amber-50 border border-amber-200 text-amber-700 rounded-xl flex items-center gap-3">
            <AlertCircle size={20} />
            <span>{masterListError}</span>
            <button
              onClick={() => setMasterListError(null)}
            ...
            {loadingCoupons || loading ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {coupons.map((c) => (
                  <div
                    key={c.id}
                    className="group relative bg-white rounded-2xl border border-slate-200 p-5 hover:shadow-lg hover:border-slate-300 transition-all"
                  >
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-slate-500 capitalize">
                          {c.discountType.toLowerCase()} discount
                        </p>
                      </div>
                    </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                {offers.map((o) => {
                  ...
                  return (
                    <div
                      key={o.id}
                      className="group relative bg-white rounded-2xl border border-slate-200 overflow-hidden hover:shadow-xl hover:border-slate-300 transition-all"
                    >
                        ...
                        <button
                          onClick={() => {
                            ...
                            setOfferForm({
                              ...
                              sponsorTypeIds: o.sponsorTypeIds || [],
                              boothIds: o.boothIds || [],
                              membershipPlanIds:
                                o.membershipPlanIds || [],
                            });
                          ...
                          className="p-2 rounded-lg bg-white/80 hover:bg-white text-slate-400 hover:text-slate-600 shadow-sm"
                        >
                          <Edit size={16} />
                        </button>
                        <button
                          onClick={() => handleOfferDelete(o.id)}
                          className="p-2 rounded-lg bg-white/80 hover:bg-white text-slate-400 hover:text-red-500 shadow-sm"
                        >
                          <Trash2 size={16} />
                        </button>
                      </div>
                      <div className="p-5">
                        <div className="flex items-start justify-between mb-4">
                          <div
                            className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-medium border ${getScopeColor(
                              o.scope
                            )}`}
                          >
                            <ScopeIcon size={14} />
                        ...
                        {o.code && (
                          <div className="inline-flex items-center gap-1.5 px-2 py-1 bg-slate-100 rounded-md text-xs font-mono text-slate-600 mb-2">
                            <Tag size={12} />
                            {o.code}
                          </div>
                        )}
                        ...
                        {o.scope === "CUSTOM" && (
                          <div className="mt-3 flex flex-wrap gap-2 text-xs">
                            {o.hotelIds && o.hotelIds.length > 0 && (
                              <span className="px-2 py-1 bg-blue-50 text-blue-600 rounded-full">
                                Hotels: {o.hotelIds.length}
                              </span>
                            )}
                            {o.ticketIds && o.ticketIds.length > 0 && (
                              <span className="px-2 py-1 bg-green-50 text-green-600 rounded-full">
                                Tickets: {o.ticketIds.length}
                              </span>
                            )}
                            {o.sponsorTypeIds &&
                              o.sponsorTypeIds.length > 0 && (
                                <span className="px-2 py-1 bg-amber-50 text-amber-600 rounded-full">
                                  Sponsors: {o.sponsorTypeIds.length}
                                </span>
                              )}
                            {o.boothIds && o.boothIds.length > 0 && (
                              <span className="px-2 py-1 bg-cyan-50 text-cyan-600 rounded-full">
                                Booths: {o.boothIds.length}
                              </span>
                            )}
                            {o.membershipPlanIds &&
                              o.membershipPlanIds.length > 0 && (
                                <span className="px-2 py-1 bg-rose-50 text-rose-600 rounded-full">
                                  Subscriptions:{" "}
                                  {o.membershipPlanIds.length}
                                </span>
                              )}
                          </div>
                        )}
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
              ...
              <div className="p-5 space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none font-mono uppercase"
                  />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none bg-white"
                    >
              ...
              <div className="flex gap-3 p-5 bg-slate-50 border-t border-slate-100">
                <button
                  onClick={resetCouponForm}
                  className="flex-1 px-4 py-2.5 border border-slate-200 rounded-xl font-medium text-slate-600 hover:bg-white transition-all"
                >
                  Cancel
                </button>
                <button
                  onClick={handleCouponSubmit}
                  className="flex-1 px-4 py-2.5 bg-gradient-to-r from-indigo-600 to-violet-600 text-white rounded-xl font-medium hover:from-indigo-700 hover:to-violet-700 transition-all"
                >
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setOfferForm({
                          ...offerForm,
                          scope: e.target.value as OfferScope,
                        })
                      }
                      className="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none bg-white"
                    >
                          ...
                          <div className="space-y-1.5 max-h-32 overflow-auto">
                            {hotels.map((h) => (
                              ...
                              >
                                <input
                                  type="checkbox"
                                  checked={offerForm.hotelIds.includes(
                                    h.id
                                  )}
                                  onChange={() =>
                                    toggleSelection("hotelIds", h.id)
                                  }
                          ...
                          <div className="space-y-1.5 max-h-32 overflow-auto">
                            {tickets.map((t) => (
                              ...
                              >
                                <input
                                  type="checkbox"
                                  checked={offerForm.ticketIds.includes(
                                    t.id
                                  )}
                                  onChange={() =>
                                    toggleSelection("ticketIds", t.id)
                                  }
                                  className="w-4 h-4 text-indigo-600 rounded border-slate-300 focus:ring-indigo-500"
                                />
                                <span className="text-sm text-slate-600">
                          ...
                          <div className="space-y-1.5 max-h-32 overflow-auto">
                            {sponsorTypes.map((s) => (
                              ...
                              >
                                <input
                                  type="checkbox"
                                  checked={offerForm.sponsorTypeIds.includes(
                                    s.id
                                  )}
                                  onChange={() =>
                                    toggleSelection(
                          ...
                          <div className="space-y-1.5 max-h-32 overflow-auto">
                            {booths.map((b) => (
                              ...
                              >
                                <input
                                  type="checkbox"
                                  checked={offerForm.boothIds.includes(
                                    b.id
                                  )}
                                  onChange={() =>
                                    toggleSelection("boothIds", b.id)
                                  }
                                  className="w-4 h-4 text-indigo-600 rounded border-slate-300 focus:ring-indigo-500"
                                />
                                <span className="text-sm text-slate-600">
                          ...
                          <div className="space-y-1.5 max-h-32 overflow-auto">
                            {membershipPlans.map((m) => (
                              ...
                              >
                                <input
                                  type="checkbox"
                                  checked={offerForm.membershipPlanIds.includes(
                                    m.id
                                  )}
                                  onChange={() =>
                                    toggleSelection(
